...
export function FileManager({ accountId, userId, projectId }: FileManagerProps) {
  const supabase = createClient();

  const [currentFolderId, setCurrentFolderId] = useState<string | null>(null);
  const [breadcrumbs, setBreadcrumbs] = useState<Breadcrumb[]>([
    { id: null, name: projectId ? "Project files" : "Office files" },
  ]);
...
>(null);
  ...
  const load = useCallback(async () => {
    ...
    folderQuery = projectId
      ? folderQuery.eq("project_id", projectId)
      : folderQuery.is("project_id", null);
    ...
    let fileQuery = supabase
      ...
      .order("name", { ascending: true });
    fileQuery = projectId
      ? fileQuery.eq("project_id", projectId)
      : fileQuery.is("project_id", null);
    ...
    setLoading(false);
  }, [supabase, accountId, projectId, currentFolderId]);
            ...
            <div key={file.id} className="flex items-center gap-3 px-3 py-2.5 hover:bg-muted/50">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => openRename("file", file.id, file.name)}>
                    <Pencil className="h-3.5 w-3.5" />
                    Rename
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
